...
In order to make it visible to the world, your local ``METADATA`` needs to be merged upstream into the official repo.
You can do so easily with the ``Pkg.publish()`` command, which publishes your local metadata changes::
...
Following the link will let you make a pull request to the official ``METADATA``.
...
As with ``Pkg.register``, these changes to ``METADATA`` aren't available to anyone else until they've been included upstream.
Again, use the ``Pkg.publish()`` command to make these changes available upstream.  
``Pkg.publish()`` makes sure that individual package repos have been tagged, pushes them if they haven't already been, and then pushes ``METADATA`` to the origin::
